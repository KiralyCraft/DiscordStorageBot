...
import com.kiralycraft.dsb.debug.Debug;
...
public class Utils {
    ...
    public static ServerSocket createServer(int port, int backlog, InetAddress address, SSLContext context, boolean ssl) throws IOException {
        ServerSocket toReturn = new ServerSocket();
        toReturn.setReceiveBufferSize(Debug.bufferSize);
        toReturn.bind(new InetSocketAddress(address, port), backlog);
        return toReturn;
    }
