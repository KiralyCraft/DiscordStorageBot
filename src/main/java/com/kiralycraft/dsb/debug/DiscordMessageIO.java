	...
	public boolean updateRawChunkData(EntityID eid, String newData) throws IOException
	{
		...
		} catch (Exception e)
		{
			...
			System.err.println("Expected JDA for messageID: " + eid.getEntityID() + " ("
					+ Debug.jdaList.indexOf(getJDA(eid.getEntityID()))
					+ ") does not correspond to the actual JDA! Attempting recover");
	...
	public EntityID createEmptyChunk(String emptyChunkDate) throws IOException
	{
		...
		while (true)
		{
			...
			while (true)
			{
				...
				if (getMillisFromID(predictedSnowFlake)
						% Debug.BOT_LINGER < (Debug.BOT_LINGER * Debug.BOT_LINGER_TIMEFRAME_SYNC))
				{
					break;
				} else
				{
