...
public interface MessageAction extends RestAction<Message>, Appendable
{
    ...
    {
        return append(csq, 0, csq.length(),UnicodeMeasurement.getActualLength(csq));//FIXED
    }
    ...
    @Nonnull
    @CheckReturnValue
    MessageAction append(@Nullable final CharSequence csq, final int start, final int end, int actualUnicodeLength); //FIXED
