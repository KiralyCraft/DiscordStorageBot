...
public class EmbedBuilder {
    ...
    public MessageEmbed build() {
        ...
        if (UnicodeMeasurement.getActualLength(description) > MessageEmbed.TEXT_MAX_LENGTH)
        ...
        final String description = UnicodeMeasurement.getActualLength(this.description) < 1 ? null : this.description.toString();
     ...
     */
    public int length() {
        int length = UnicodeMeasurement.getActualLength(description);
        synchronized (fields) {
            for (MessageEmbed.Field f : fields) {
                int name = UnicodeMeasurement.getActualLength(f.getName());
                int val = UnicodeMeasurement.getActualLength(f.getValue());
                length += name + val;
            }
            //  length = fields.stream().map(f -> UnicodeMeasurement.getActualLength(f.getName()) + UnicodeMeasurement.getActualLength(f.getValue())).reduce(length, Integer::sum);
        }
        if (title != null) {
            length += UnicodeMeasurement.getActualLength(title);
        }
        if (author != null) {
            length += UnicodeMeasurement.getActualLength(author.getName());
        }
        if (footer != null) {
            length += UnicodeMeasurement.getActualLength(footer.getText());
        }
        return length;
    }
    ...
    public EmbedBuilder setTitle(@Nullable String title, @Nullable String url) {
        if (title == null) {
            ...
            Checks.check(UnicodeMeasurement.getActualLength(title) <= MessageEmbed.TITLE_MAX_LENGTH, "Title cannot be longer than %d characters.", MessageEmbed.TITLE_MAX_LENGTH);
    ...
    public final EmbedBuilder setDescription(@Nullable CharSequence description) {
        ...
        if (description != null && UnicodeMeasurement.getActualLength(description) >= 1)
    ...
    public EmbedBuilder appendDescription(@Nonnull CharSequence description) {
        ...
        Checks.check(UnicodeMeasurement.getActualLength(this.description) + UnicodeMeasurement.getActualLength(description) <= MessageEmbed.TEXT_MAX_LENGTH,
    ...
    public EmbedBuilder setFooter(@Nullable String text, @Nullable String iconUrl) {
        ...
        if (text == null) {
            ...
            Checks.check(UnicodeMeasurement.getActualLength(text) <= MessageEmbed.TEXT_MAX_LENGTH, "Text cannot be longer than %d characters.", MessageEmbed.TEXT_MAX_LENGTH);
            urlCheck(iconUrl);
