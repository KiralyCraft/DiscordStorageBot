...
public class MessageActionImpl extends RestActionImpl<Message> implements MessageAction
{
    ...
    {
        super(api, route);
        Checks.check(UnicodeMeasurement.getActualLength(contentBuilder) <= Message.MAX_CONTENT_LENGTH,
    ...
    public MessageActionImpl content(final String content)
    {
        ...
        else if (UnicodeMeasurement.getActualLength(content) <= Message.MAX_CONTENT_LENGTH)
            this.content.replace(0, content.length(), content); //This should stay, as StringBuilder still uses old measurements
        else
    ...
    @Override
    @CheckReturnValue
    public MessageActionImpl append(final CharSequence csq, final int start, final int end,int actualUnicodeLength)
    {
        if (UnicodeMeasurement.getActualLength(content) + actualUnicodeLength - start > Message.MAX_CONTENT_LENGTH) //FIXED
    ...
    {
        if (UnicodeMeasurement.getActualLength(content) == Message.MAX_CONTENT_LENGTH)
            throw new IllegalArgumentException("A message may not exceed 2000 characters. Please limit your input!");
        content.append(c);
        return this;
    }
    ...
    protected DataObject getJSON()
    {
        ...
        if (override)
        {
            if (embed == null)
                ...
                obj.put("embed", embed);
            if (UnicodeMeasurement.getActualLength(content) == 0)
                obj.putNull("content");
            else
	...
	{
		throw new RuntimeException("Not implemented"); //FIXED
	}
}
